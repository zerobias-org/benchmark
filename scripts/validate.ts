...
import { UUID, URL, Severity } from '@auditmation/types-core-js';
...
function processPackageJson(packageFile: Record<string, any>, code: string): void {
  let check: any = packageFile.name !== undefined && packageFile.name !== null
    && packageFile.name === `@zerobias-org/benchmark-${code.replace('_', '-').replace('.', '_')}` ? true
    : new Error('package.json missing name or not set to @zerobias-org/benchmark-<codeWithDashes>');
  ...
  if (packageFile.auditmation && typeof packageFile.auditmation === 'object') {
    ...
    check = auditmation['import-artifact'] !== undefined && auditmation['import-artifact'] !== null && auditmation['import-artifact'] === 'benchmark'
      ? true : new Error('package.json auditmation section missing import-artifact or not set to benchmark');
    check = auditmation.package !== undefined && auditmation.package !== null && auditmation.package === `zerobias.${codeSplit.join('.')}.benchmark`
      ? true : new Error(`package.json auditmation section missing package or not set to zerobias.${codeSplit.join('.')}.benchmark`);
...
async function processIndexYml(indexFile: Record<string, any>): Promise<string> {
  ...
  let check: any;
  check = indexFile.standardType !== undefined && indexFile.standardType !== null && indexFile.standardType === 'benchmark'
   ? indexFile.standardType : new Error('standardType not found in index.yml or not equal to benchmark');
...
async function processElements(directory: string, elementCodes: string[]): Promise<void> {
  for (const elementCode of elementCodes) {
    ...
    check = elementYml.severity !== undefined && elementYml.severity !== null && Severity.from(elementYml.severity)
      ? elementYml.severity : new Error('severity not found in elements/${code}.yml');
